...
- **Rule #6**: In `PROGRESS.md`, ONLY append new rows for new code submissions or code-modifying reviews. Do NOT add new rows to `PROGRESS.md` for Q&A-only updates (when the user only answers conceptual follow-up questions without modifying their code). NEVER replace or overwrite an existing row; every code-modifying review iteration must have its own entry to show the learning curve.
